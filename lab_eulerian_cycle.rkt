...
(define (degree relation vertex)
  (cond ((null? relation) 0)
        ((equal? (car (car relation)) vertex) (cond ((equal? (car (cdr (car relation))) vertex) (+ 2 (degree (cdr relation) vertex))) (else (+ 1 (degree (cdr relation) vertex)))))
        (else (degree (cdr relation) vertex))))
...
(define (maxVertex relation)
   (maxVertexs relation 1))

(define (maxVertexs relation unit)
  (cond ((null? relation) unit)
        ((> (car (car relation)) unit) (cond ((> (car (cdr (car relation))) (car (car relation))) (maxVertexs (cdr relation) (car (cdr (car relation))))) (else (maxVertexs (cdr relation) (car (car relation))))))
        ((> (car (cdr (car relation))) unit) (maxVertexs (cdr relation) (car (cdr (car relation)))))
        (else (maxVertexs (cdr relation) unit))))
...
(define (has-eulerian-cycle? relation)
  (euler relation (maxVertex relation)))

(define (euler relation unit)
  (cond ((equal? 0 unit) #t)
        ((even? (degree relation unit)) (euler relation (- unit 1)))
        (else #f)))

...
(define (reflexive? relation n)  
    (cond ((equal? (intersection (id n) relation) (id n)) #t)
          (else #f)))
...
(define (flip-pairs relation)
    (cond ((null? relation) empty)
          (else (append (list (reverse (car relation))) (flip-pairs (cdr relation))))))
...
(define (symmetric? relation)
   (cond ((set-equal? (flip-pairs relation) relation) #t)
         (else #f)))
...
(define (symmetric-closure relation)
  (cond ((symmetric? relation) relation)
        (else (make-set (append (flip-pairs (sym-diff relation (flip-pairs relation))) relation)))))
...
(define (related-to element relation)
   (cond ((null? relation) '())
         (else (cond ((equal? (car (car relation)) element) (cons (car (cdr (car relation))) (related-to element (cdr relation))))
                     (else (related-to element (cdr relation)))))))

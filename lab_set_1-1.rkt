...
(define (element? item list-of-items)
  (cond ((null? list-of-items) #f)
        ((equal? item (car list-of-items)) #t)
        (else (element? item (cdr list-of-items)))))
...
(define (intersection listA listB)
  (make-set (cond ((null? listA) '())
        ((element? (car listA) listB) (cons (car listA) (intersection (cdr listA) listB)))
        (else (intersection (cdr listA) listB)))))
...
(define (subset? setA setB)
  (equal? (make-set setA) (intersection setA setB)))
...
(define (set-difference listA listB)
  (make-set (cond ((null? listA) '())
        ((element? (car listA) listB) (set-difference (cdr listA) listB))
        (else (cons (car listA) (set-difference (cdr listA) listB))))))
...
(define (cardinality lst)
  (cardinality2 (make-set lst)))
(define (cardinality2 lst)
  (cond ((null? lst) 0)
        (else (+ 1 (cardinality2 (cdr lst))))))
...
(define (disjoint? setA setB)
  (set-equal? (append setA setB) (sym-diff setA setB)))
...
;;  (cond ((null? set) '())
;;        ((equal? (car set) element) (remove element (cdr set)))
;;        (else (cons element (remove element (cdr set))))))

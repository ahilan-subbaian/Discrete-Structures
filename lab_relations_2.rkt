    ...
    (if (null? relationInner)
      ...
      (if (null? relationOuter)
          ...
          (if (equal? (car (cdr (car relationInner))) (car (car relationOuter)))
              (make-set (append (list (list (car (car relationInner)) (car (cdr (car relationOuter))))) (compositeH  (cdr relationOuter) relationInner outerInitial)))
...
(define (power relation k)
  (powerH relation k relation))

(define (powerH relation k store)
  (if (= k 0)
      '()
      (if (= k 1)
          store 
      (powerH relation (- k 1) (composite relation store)))))
...
(define (transitive? relation)
   (set-equal? relation (composite relation relation)))
...
(define (EQ-relation? relation n)
  (and (and (reflexive? relation n) (symmetric? relation)) (transitive? relation)))

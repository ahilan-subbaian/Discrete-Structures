...
(define (helper number base list)
  (cond ((= number 0) list)
        (else (helper (quotient number base) base (cons (modulo number base) list)))))
...
(define (base10 oldbase numlist)
  (cond ((null? numlist) 0)
        (else (+ (* (car numlist) (expt oldbase (- (lnth numlist) 1))) (base10 oldbase (cdr numlist))))))
...
(define (geo-prog cur_num ratio n)
  (cond ((or (or (= cur_num 0) (= ratio 0)) (= n 0)) '())
        (else (cons cur_num (geo-prog (* cur_num ratio) ratio (- n 1))))))